...
import ArtistScreen from './ArtistScreen'; // 🔴 IMPORT THE NEW ARTIST SCREEN
...
export default function HomeScreen({ onOpenSubPage, playQueue }) {
  ...
  // 🔴 STATE FOR OPENING ARTIST PROFILE
  ...
  return (
    <div className="w-full flex flex-col pt-2 pb-0 animate-fade-in relative overflow-x-hidden">
        ...
        <div className="mb-6 animate-fade-in">
          ...
          <div className="flex overflow-x-auto no-scrollbar gap-6 px-6 min-h-[120px]">
            {artists.map((artist) => (
              <div 
                key={artist.id} 
                onClick={() => setSelectedArtist(artist)} // 🔴 OPENS THE ARTIST SCREEN
                className="flex flex-col items-center gap-2 shrink-0 cursor-pointer active:scale-90 transition-transform"
              >
      ...
      {/* 🔴 RENDER ARTIST SCREEN WHEN AN ARTIST IS CLICKED */}
      {selectedArtist && (
        <ArtistScreen 
