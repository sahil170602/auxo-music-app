...
export default function Player({ showBottomNav = true }) { 
  ...
  useEffect(() => {
    if ('mediaSession' in navigator && currentSong) {
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        album: 'AUXO',
        artwork: [
          { src: currentSong.image_url, sizes: '512x512', type: 'image/webp' }
        ]
      });
  ...
  return createPortal(
    <div id="auxo-player">
      ...
      {/* ================== MINI PLAYER ================== */}
      {/* 🔴 FIX: Increased z-[600] to z-[99999] so it always floats on top of portaled screens */}
      {!isExpanded && (
        <div 
          onClick={openExpandedPlayer}
          className={`fixed left-4 right-4 h-16 bg-neutral-900/90 backdrop-blur-xl border border-white/10 rounded-2xl flex items-center px-4 gap-4 z-[99999] animate-slide-up-mini shadow-2xl cursor-pointer overflow-hidden transition-all duration-300 ${
            showBottomNav 
      ...
      {/* ================== FULL PLAYER SCREEN ================== */}
      {/* 🔴 FIX: Increased z-[1000] to z-[100000] */}
      {isExpanded && (
        <div className="fixed inset-0 bg-neutral-950 z-[100000] flex flex-col animate-slide-up-full overflow-hidden overscroll-contain">
          ...
          {showLyrics && (
            <div 
              ...
              className="fixed inset-0 z-[200000] bg-neutral-950/95 backdrop-blur-3xl flex flex-col animate-fade-in overflow-hidden touch-none overscroll-contain"
            >
    ...
    document.body // 🔴 3. Mounts Player to the very top of the DOM
  );
}